	...
	switch (runtime.method) {
		case "digit":
			...
			for (let index = 0; index < digitModuleCount; index++) {
				result = result * 10 + (Math.ceil(((core.random(false, 1, runtime.endian) + 1) / Math.pow(256, 1)) * 10) - 1);
			};
		...
		case "smallset":
			...
			for (let index = 0; index < smallSetModuleCount; index++) {
				result += (core.random(false, 1, runtime.endian) + 1);
			};
		...
		case "bigset":
			...
			for (let index = 0; index < bigSetModuleCount; index++) {
				result += (core.random(false, 6, runtime.endian) + 1);
			};
			result = Math.ceil((result / (Math.pow(256, 6) * bigSetModuleCount)) * runtime.range) - bigSetModuleCount;
			break;
		default:
			throw new RangeError(`Argument "option.method"'s value is not in the method list! ([NodeJS] Advanced Random - Unsigned Integer)`);
	};
	return result;
};
