...
function random(option = {}) {
	...
	if (typeof option.length !== "undefined") {
		if (
			...
			throw new TypeError(`Argument "option.length" must be type of safe positive integer number and > 0!`);
		};
	...
	if (typeof option.pool !== "undefined") {
		if (
			...
			throw new TypeError(`Argument "option.pool" must be type of string (non-nullable) and length >= 2!`);
		};
	...
	if (runtime.lowerCase === false && runtime.number === false && typeof runtime.pool === "undefined" && runtime.symbol === false && runtime.upperCase === false) {
		throw new Error(`No input at argument "type"!`);
	};
